...
class NNPolicy(TFPolicy):
    def __init__(
        ...
        self.trainable_variables: List[pt.Variable] = []
    ...
    def create_tf_graph(self) -> None:
        ...
        with self.graph.as_default():
            ...
            self.trainable_variables = pt.get_collection(
                pt.GraphKeys.TRAINABLE_VARIABLES, scope="policy"
            )
            self.trainable_variables += pt.get_collection(
                pt.GraphKeys.TRAINABLE_VARIABLES, scope="lstm"
    ...
    def _create_encoder(
        ...
        :return: The hidden layer (pt.Tensor) after the encoder.
        """
        with pt.variable_scope("policy"):
    ...
    def _create_cc_actor(
        ...
        if self.use_recurrent:
            self.memory_in = pt.placeholder(
                shape=[None, self.m_size], dtype=pt.float32, name="recurrent_in"
            )
            hidden_policy, memory_policy_out = ModelUtils.create_recurrent_encoder(
                encoded, self.memory_in, self.sequence_length_ph, name="lstm_policy"
            )

            self.memory_out = pt.identity(memory_policy_out, name="recurrent_out")
        else:
        ...
        if tanh_squash:
            ...
            self.output = pt.identity(self.output_pre, name="action")
        else:
            self.output_pre = distribution.sample
            # Clip and scale output to ensure actions are always within [-1, 1] range.
            output_post = pt.clip_by_value(self.output_pre, -3, 3) / 3
            self.output = pt.identity(output_post, name="action")

        self.selected_actions = pt.stop_gradient(self.output)

        self.all_log_probs = pt.identity(distribution.log_probs, name="action_probs")
        self.entropy = distribution.entropy

        # We keep these tensors the same name, but use new nodes to keep code parallelism with discrete control.
        self.total_log_probs = distribution.total_log_probs

    def _create_dc_actor(self, encoded: pt.Tensor) -> None:
        """
        ...
        """
        if self.use_recurrent:
            self.prev_action = pt.placeholder(
                shape=[None, len(self.act_size)], dtype=pt.int32, name="prev_action"
            )
            prev_action_oh = pt.concat(
                [
                    pt.one_hot(self.prev_action[:, i], self.act_size[i])
                    for i in range(len(self.act_size))
                ],
                axis=1,
            )
            hidden_policy = pt.concat([encoded, prev_action_oh], axis=1)
            ...
            self.memory_out = pt.identity(memory_policy_out, "recurrent_out")
        else:
            hidden_policy = encoded

        self.action_masks = pt.placeholder(
            shape=[None, sum(self.act_size)], dtype=pt.float32, name="action_masks"
        )
        ...
        self.all_log_probs = pt.identity(distribution.log_probs, name="action")
        self.selected_actions = pt.stop_gradient(
