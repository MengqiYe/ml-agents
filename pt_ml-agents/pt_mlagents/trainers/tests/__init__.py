...
if os.getenv("TEST_ENFORCE_NUMPY_FLOAT32"):
    ...
    def _check_no_float64(arr, kwargs_dtype):
        if arr.dtype == np.float64:
            ...
            # Only raise if this came from pt_mlagents code, not pytorch
            if (
                "ml-agents/pt_mlagents" in filename
                or "ml-agents-envs/pt_mlagents" in filename
            ) and "pytorch_to_barracuda.py" not in filename:
                raise ValueError(
