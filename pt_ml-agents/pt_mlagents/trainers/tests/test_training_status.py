...
from pt_mlagents.trainers.training_status import (
    StatusType,
    StatusMetaData,
...
class StatsMetaDataTest(unittest.TestCase):
    def test_metadata_compare(self):
        # Test write_stats
        with self.assertLogs("pt_mlagents.trainers", level="WARNING") as cm:
            default_metadata = StatusMetaData()
            version_statsmetadata = StatusMetaData(pt_mlagents_version="test")
            default_metadata.check_compatibility(version_statsmetadata)

            pt_version_statsmetadata = StatusMetaData(tensorflow_version="test")
            default_metadata.check_compatibility(pt_version_statsmetadata)
